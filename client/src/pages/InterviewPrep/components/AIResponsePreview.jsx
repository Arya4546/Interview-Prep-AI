            ...
            img({ src, alt }) {
              return (
                <img
                  src={src}
                  alt={alt}
                  className='my-4 max-w-full rounded shadow-sm hover:shadow-lg transition'
                />
              );
            },
          }}
...
function CodeBlock({ code, language }) {
  ...
  return (
    <div className='relative my-6 rounded-xl overflow-hidden bg-gray-50 border border-gray-200 shadow-sm hover:shadow-md transition'>
      <div className='flex items-center justify-between px-4 py-2 bg-gray-100 border-b border-gray-200'>
        ...
        <button
          onClick={copyCode}
          className='text-gray-500 hover:text-gray-700 focus:outline-none relative group transition'
          aria-label='Copy code'
        >
          {copied ? (
            <LuCheck size={16} className='text-green-600 transition-transform duration-200 scale-110' />
          ) : (
            <LuCopy size={16} />
          )}
          <span
            className={`absolute -top-8 right-0 bg-black text-white text-xs rounded-md px-2 py-1 opacity-0 group-hover:opacity-80 transition-all duration-200 ${
              copied ? 'opacity-100' : ''
            }`}
          >
            Copied!
